...
public class Adb {
	...
	/**
	 * Recursively scans the specified USB hub for ADB devices and puts them into
	 * the list.
	 * 
	...
	public static UsbDevice findDevice(short vendorId, short productId) throws SecurityException, UsbException {
		...
		// System.out.println("** Se recibe hub" + hub + ", vId: " + vendorId + ", pId:
		// " + productId);
	...
	/**
	 * Checks if the specified USB device is a ADB device and adds it to the list if
	 * it is.
	 * 
	 * @param usbDevice  The USB device to check.
	 * @param adbDevices The list of ADB devices to add the USB device to when it is
	 *                   an ADB device.
	 */
	@SuppressWarnings("unchecked")
	private static void checkDevice(UsbDevice usbDevice, List<AdbDevice> adbDevices) {
		UsbDeviceDescriptor deviceDesc = usbDevice.getUsbDeviceDescriptor();

		// Ignore devices from Non-ADB vendors
		if (!isAdbVendor(deviceDesc.idVendor()))
			return;
	...
	private static boolean isAdbInterface(UsbInterface iface) {
		...
		return desc.bInterfaceClass() == ADB_CLASS && desc.bInterfaceSubClass() == ADB_SUBCLASS
	...
	public static void showUsbControlIrpCommunication(UsbDevice usbDevice) {
		/*
		 * UsbControlIrps have 2 parts: a header, or 'setup packet' in low-level USB
		 * terms, and data buffer. The header tells the device what control action the
		 * communication is requesting. These particular values will perform a
		...
		/*
		 * All communication on the DCP (and all control-type pipes) is done using
		 * UsbControlIrp objects. There are 3 different ways to create a UsbControlIrp
		 * object: (1) Call the UsbDevice.createUsbControlIrp() method. (2) Use the
		 * javax.usb.util.DefaultUsbControlIrp class. (3) Define your own class that
		 * implements the UsbControlIrp interface. The first way is the best (but least
		 * flexible) as it produces a UsbControlIrp that may be optimized to whatever
		 * javax.usb implementation you are currently using; however you are limited to
		 * only those methods provided by the UsbControlIrp interface. The second way is
		 * more flexible, as you can use all the methods defined by the
		 * DefaultUsbControlIrp class, but it may not be as optimized to the
		 * implementation (i.e. the javax.usb implementation may have to do more work to
		 * process it). The third way is the most flexible as you can put whatever you
		 * want into your class; however this way will very likely require more
		 * processing and/or memory by the javax.usb implementation. A combination of
		 * the second and third options is also possible, where you create your own
		 * class that extends the DefaultUsbControlIrp class. For this example we will
		...
		/*
		 * This is the number of bytes actually received from the device. If the data
		 * direction was out (host-to-device), this would be the number of bytes
		 * actually sent to the device. For the input (device-to-host) case, this may be
		 * less than the length of the provided buffer (providing the irp is set to
		 * accept short packets). For the output case, this should never be less than
		 * the size of the provided buffer (but you may want to check anyway to be
		 * sure!).
		 */
		int length = usbControlIrp.getActualLength();

		/*
		 * The device descriptor is binary, as specified by the USB spec. We're not
		 * going to parse it here, but we can print it out.
		 */
